...
class Params:
    ...
    print_freq = 20
    save_freq = 200
    model_path = './model/unet_1718_KidT'
    txt_postfix = 'unet_1718_KidT' + '_' + '_'.join(modal_dict.keys())
    data_dir = DataPath.preprocessed_training_data_folder
    data_argument = False

    # ---Testing setting
    saved_model = './model/unet_1718_KidT/model_4'
    save_prob = False
    test_save_dir = './results/unet_1718_KidT/'
...
def train(device):
    ...
    model.train()
    model.to(device)
    train_idxs = list(range(48))
    train_data = load_dataset(train_idxs)
    train_data_17 = load_dataset(train_idxs, "/data/ljf/kidney_tumor_nii_2_pprocess/train")
    for x in train_idxs:
        train_data_17[x + 48] = train_data_17.pop(x)
