...
class Params:
    run_mode = 'test'  # ---'train' or 'test' or testprob or fusion
...
def test(device):
    ...
    for idx in Params.test_data_idxs:
        ...
        new_seg = np.zeros(label.shape, label.dtype)
        new_seg[label == 2] = 1
        new_seg[label == 1] = 2
        # convert label 4 enhancing tumor to label 3
        new_seg[label == 3] = 3
        save_image(new_seg, f"{Params.test_save_dir}/{data['name']}.nii.gz")
