...
#                     for name in files if 'seg' in name and 'FAST' in name 
...
#                     for name in files if brainParcellationName in name and 'lab' not in name 
#                      and 'threshold' in root and name.endswith('.nii.gz')]
...
def Brats2018FilePaths(bratsPath):
    ...
    flair_filepaths = [os.path.join(root, name) for root, dirs, files in os.walk(bratsPath)
                    for name in files if 'flair' in name and 'corrected' in name 
                    and 'normalized' not in name and 'MNI152' not in name and name.endswith('.nii.gz')]
    flair_filepaths.sort()

    seg_filepaths = [os.path.join(root, name) for root, dirs, files in os.walk(bratsPath)
                    for name in files if 'seg' in name and 'FAST' not in name 
                    and 'MNI152' not in name and name.endswith('.nii.gz')]
    seg_filepaths.sort()

    assert (len(t1_filepathes) == len(t1c_filepathes) == len(t2_filepathes) == len(flair_filepathes) 
                    == len(seg_filepathes)), "The len of different image modalities are differnt!!!"
...
def Brats2018OriginalFilePaths(bratsPath):
    ...
    t2_filepaths = [os.path.join(root, name) for root, dirs, files in os.walk(bratsPath)
                    for name in files if 't2' in name and 'corrected' not in name 
                    and 'normalized' not in name and 'MNI152' not in name and name.endswith('.nii.gz')]
    t2_filepaths.sort()

    flair_filepaths = [os.path.join(root, name) for root, dirs, files in os.walk(bratsPath)
                    for name in files if 'flair' in name and 'corrected' not in name 
                    and 'normalized' not in name and 'MNI152' not in name and name.endswith('.nii.gz')]
    flair_filepaths.sort()
    ...
    assert len(t1_filepaths)==len(t1c_filepaths)==len(t2_filepaths)==len(
                    flair_filepaths)==len(seg_filepaths), "Lengths are different!!!"
...
def BrainMaskPaths(bratsPath):
    ...
    brain_mask_paths = [os.path.join(root, name) for root, dirs, files in os.walk(bratsPath)
                    for name in files if 'brainmask' in name and name.endswith('.nii.gz')]
