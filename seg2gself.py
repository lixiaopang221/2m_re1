...
class Params:
    ...
    pre_folder_ls = ["01to01", "23to23"] #--- ["t1_t1ce_t2_flair"]
    model_xy      = 'x2y'
    model_yx      = 'x2y'
    print_freq    = 20
    save_freq     = 200
    model_path    = './model/gan_22gself'
    txt_postfix   = 'gan_22gself' + '_' + '_'.join(modal_dict.keys())
    data_dir      = DataPath.preprocessed_training_data_folder
    data_argument = False

    #---Testing setting
    saved_model    = './model/gan_22gself/model_4'
    test_save_dir  = './results/gan_22gself'
...
def restore_pretrain(model):
    if Params.pre_folder_ls is None:
        return model
    def get_latest(mode, i:int):
        paths = sorted(glob(f"{Params.pretrain_dir}/{Params.pre_folder_ls[i]}/{mode}*"))
        return paths[-1]
    def get_pre_dict(net, mode, i:int):
        latest_saved_model_path = get_latest(mode, i)
        print(latest_saved_model_path)
        saved_state_dict = torch.load(latest_saved_model_path)
    ...
    model.net_0.load_state_dict(get_pre_dict(model.net_0, Params.model_xy, 0))
    model.net_1.load_state_dict(get_pre_dict(model.net_1, Params.model_yx, 1))
    return model
